---
title: "Konktrakty SWAP"
author: "Jakub Lipski"
date: "22 pazdziernika 2015"
...
## Definicja
  ...
  Kontrakty *SWAP* na rynkach pojawiły się w początku lat 80. ubiegłego stulecia. Od tego czasu a zwłaszcza w ostatnich kilkunastu latach obserwuje sie dynamiczny rozwoj tych instrumentów. Obecnie stały sie one najczęsciej stosowanym instrumentami transferu ryzyka.
...
Transakcje swap tworzą wraz z transakcjami forward, opcjami, kontraktami futures rodzinę instrumentów pochodnych, zwanych derywatami. 
  ...
  Kontrakty *SWAP* występują w zasadzie jedynia poza giełdami.Można je traktować jako ougólnienie kontraktu forward a ściślej jako portfel (złożenie) kontraktów forward o różnych terminach realizacji.
Należy zwrócic uwagę, że isniteje wiele różnych rodzajów kontraktów dla których przyjęta jest nazwa *swap*, nie jest przeto łatwe podanie uniwersalnej definicji tego instrumentu. Przyjmę uproszczone określenie dotyczące podstawowych rodzajów *swap*.
  Kontrakt *swap* jest to instrument finansowy w którym każda ze stron dokonuje w ustalonych terminach w przyszłości serii płatności na rzecz drugiej strony kontraktu przy czym co najmniej jedna seria płatności zależy od wartości indeksu podstawowego. Dodajmy, że często w ustalonych terminach występuje tylko jeden przepływ pieniężny będący różnicą między przepływami pieniężnymi płaconymi przez obie strony - jest to przepływ pieniężny netto.
  
  Jak widać określenie to jest bardzo ogólne można jednak wryróżnic dwie istotne cechy tego kontraktu:
  
1. jest więcej niż jeden ustalony termin, zwany terminem rozliczenia;
2. jest określony jako indeks podstawowy;
  
  Podobnie jak w przypadku pozostałych instrumentów pochodnych tak i tutaj indeks podstawowy odzwierciedla  rodzaj ryzyka do transferu którego *swap* jest wykorzystany.
  Istotę funkcjonowania swap przedstawię na przykładzie najbardziej popularnego kontraktu. Jest to *swap na stopę procentową* (*interest rate swap*- IRS) Indeksem podstawowym jest tutaj stopa procentowa (w tym przykładzie jest to WIBOR).
...
![image](/Users/kuba/Desktop/Projekt Eko Fin/Rysunek1.jpg)

**Rysunek 1** *Swap na stopę procentową*
...
* jesli podmiot otrzymuje płatnosci według  zmiennej stopy procentowej, a zatem jest narażony na ryzyko spadku stopy procentowej, to powinien w kontrakcie swap zająć pozycje otrzymującego  płatności według stałej stopy procentowej,
a dokonującego płatności według zmiennej stopy procentowej.
* jeżeli podmiot dokonuje płatności według zmiennej stopy procentowej, a zatem jest narażony na ryzyko wzrostu stopy procentowej to powinien w kontrakcie swap zając pozycję dokonjącego płatności według stałej stopy procentowej, a otrzymującego płatności według zmiennej stopy procentowej.

W praktyce jedną ze stron kontrkatu swap jest bank. Banki oferują te kontrakty, zajmują jednocześnie długą i krótką pozycję. Warunkiem jest to , że w obu kontraktach jest ten  sam instrument podstawowy.
Przykład, w którym bank zawarł jednocześnie dwa kontrakty swap:

```
PRZYKŁAD 2
```

Rozpatrujemy kontrkaty swap oferowane przez pewien bank. Przyjmijmy, że wartość nominalna kontraktu równa jest 100 000zł, termin wygaśnięcia to 5 lat, płatności są co pół roku, a stopa referencyjna kontraktu to WIBOR (6 - miesięczny)
Bank podaje oferty stopy kontraktu w odniesieniu do pozycji długiej i krótkiej (są to kwotowania bid/ask); wynoszą one 5,9 % oraz 6,1 %. Bank jednocześnie zajmuje długą i krótką pozycję, zawierając kontrkaty z podmiotem A i podmiotem B. 
Rysunek przedstawia płatności dokonywane przez trzy strony w dwóch kontraktach, przy czym płatności są w okresach półrocznych przez 5 lat, łącznie zatem jest 10 płatności.

![image](/Users/kuba/Desktop/Projekt Eko Fin/Rysunek2.jpg)

**Rysunek 2** *System dwóch kontraktów swap na stopę procentową*
...
**Swap na stopę procentową** jest zdecydowanie najbardziej popularnym rodzajem konktratu swap . Występują jednak także inne rodzaje kontraktów tego typu:
Swap walutowy (currency swap) jest to konktrakt w którym płatnosci obu stron są w różnych walutach a ponadto mogą być  to płatności  według  stałej lub zmiennej stopy procentowej.
Jeśli pod uwagę weźmiemy stronę A, która otrzymuje płatności w krajowej walucie to można wyróżnic cztery podstawowe rodzaje swapu walutowego:
...
Zasadę funkcjonowania swapu walutowego przedstawie na przykładzie czwartego z wymienionych powyżej rodzajów, przy czym w podanym przykładzie walutą krajową jest złoty a obcą zaś EURO.

```
PRZYKŁAD 3
```

Przedsiebiorstwo polskie zawiera kontrakt swap z bankiem. Termin wygaśnięcia tego kontraktu to 3 lata, a płatności dokonywane są co pół roku. W momencie zawarcia tego konktratu kurs walutowy wynosi 4 zł za euro. Wartość nominalna konktraktu to 4 mln zł , czyli 1 mln euro.
Stopy procentowe określone w swapie to WIBOR (dla złotego) i EURIBOR (dla euro). Schemat działania konktraktu przedstawiony został na rysunku.

![image](/Users/kuba/Desktop/Projekt Eko Fin/Rysunek3.jpg)

**Rysunek 3** *Swap walutowy*

Działanie swapu walutowego przebiega w trzech etapach. Pierwszy etap jest w momencie zawracia konktratu. Wtedy strona A płaci bankowi 4 mln zł, a otrzymuje od banku 1 mln euro - czyli następuje wymiana kapitału.
Drugi etap to dokonywanie płatności zależnych od stóp procentowych w kolejnych półroczach przez 3 lata w sumie 6 płatności. Strona A dokonuje płatności zależnych od stopy EURIBOR, w bank płatności zależnych  od stopy WIBOR. Trzeci etap jest w momencie wygaśnięcia konktraktu. Wtedy strona A płaci bankowi 1 mln euro, otrzymuje zaś od banku 4mln zł.
Jak widać płatności trzeciego etapu to odwrócenie płatności pierwszego etapu.
...
Skarbnica Wiedzy Przedsiębiorców 
<http://www.skarbiec.biz/encyklopedia/swapy.htm>
